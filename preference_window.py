...
class Window(ttk.Frame):
    ...
    def create_widgets_progresses(self) -> None:
        self.pbval = 0
        ...
        self.time_label = tk.Label(self.frame_bars, text="time (sec)")
        self.time = tk.Label(self.frame_bars, text="tobe..")
        self.event_i_label = tk.Label(self.frame_bars, text="N of events")
        self.event_i = tk.Label(self.frame_bars, text="imple")
        self.n_atoms_label = tk.Label(self.frame_bars, text="N of atoms")
        self.n_atoms = tk.Label(self.frame_bars, text="ment")
        self.coverage_label = tk.Label(self.frame_bars, text="Coverage (ML)")
        self.coverage = tk.Label(self.frame_bars, text="...")

    def create_layout_progresses(self) -> None:
        self.progress_label.grid(row=0, columnspan=5, pady=15)
        self.progress_bar.grid(row=1, columnspan=5, pady=10)
        self.time_label.grid(row=2, column=0, **self.padWE)
        self.time.grid(row=3, column=0, **self.padWE)
        self.event_i_label.grid(row=2, column=1, **self.padWE)
        self.event_i.grid(row=3, column=1, **self.padWE)
        self.n_atoms_label.grid(row=2, column=2, **self.padWE)
        self.n_atoms.grid(row=3, column=2, **self.padWE)
        self.coverage_label.grid(row=2, column=3, **self.padWE)
        self.coverage.grid(row=3, column=3, **self.padWE)

    def close_function(self) -> None:
        # plt.close("all")  <<< FIXME
        self.quit()

    def store_parameter(self) -> None:
        params["Unit_length"] = float(self.n_cell.get())
        params["z_unit"] = float(self.n_unit.get())
        params["temperature_k"] = float(self.temperature.get())
        params["deposition_rate"] = float(self.deposition_rate.get())
        params["depostition_time"] = float(self.deposition_time.get())
        params["post_anneal"] = float(self.postanneal_time.get())
        params["prefactor"] = float(self.prefactor.get())
        ##
        ...
