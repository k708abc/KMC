...
cdef rec_ppt(
    ...
    table_mode.cell(1, 0).text = str(mode_val_or[3])
    table_mode.cell(1, 1).text = str(mode_val_or[0])
    table_mode.cell(1, 2).text = str(mode_val_or[1])
    table_mode.cell(1, 3).text = str(mode_val_or[2])
    #
    #
    slide = prs.slides.add_slide(blank_slide_layout)
    width = height = Inches(1)
